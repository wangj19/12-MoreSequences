...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(115, 300),  # All numbers are approximate.
                rg.Point(100.0, 325.0),
                rg.Point(85.0, 300.0),
                rg.Point(100.0, 285.0)]
    circle = rg.Circle(rg.Point(100, 300), 15)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        center = points[k]
        s_circle = rg.Circle(points[k],10)
        s_circle.fill_color = color
        s_circle.attach_to(window)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points)-1):
        lines = rg.Line(points[k+1],points[k])
        lines.color = color
        lines.thickness = thickness
        lines.attach_to(window)
    window.render()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points)):
        if k+hops_to_next_point < len(points):
            ...
            lines = rg.Line(points[k],points[k+hops_to_next_point-len(points)])
        lines.color = color
        lines.thickness = thickness
        lines.attach_to(window)
    window.render()
